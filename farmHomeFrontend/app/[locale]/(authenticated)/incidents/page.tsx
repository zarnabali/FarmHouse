...
import { Plus, Search, Download, Upload, Edit, Trash2, Eye, AlertTriangle, Loader2 } from "lucide-react"
...
export default function IncidentsPage() {
  ...
  const fetchRecords = async (page = 1, limit = itemsPerPage) => {
    ...
    try {
      ...
      const url = new URL(`${config.backendUrl}/incidents`)
      ...
      setRecords(Array.isArray(data.incidents) ? data.incidents : data.records || [])
      setTotalPages(data.totalPages || 1)
      setTotalRecords(data.total || (Array.isArray(data.incidents) ? data.incidents.length : 0))
  ...
  const filteredRecords = records.filter((record) => {
    const matchesSearch =
      record.incidentDescription.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.reportedBy.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.affectedAnimals.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesType = filterType === "all" || record.incidentType === filterType
    const matchesStatus = filterStatus === "all" || record.incidentStatus === filterStatus
  ...
  const handleExportCSV = () => {
    const csvContent = [
      ["Type", "Date", "Reported By", "Affected Animals", "Description", "Status", "Severity", "Cost"].join(","),
      ...filteredRecords.map((record) =>
        [
          record.incidentType,
          record.incidentDate,
          record.reportedBy,
          record.affectedAnimals,
    ...
    a.href = url
    a.download = "incident-records.csv"
    a.click()
  }
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
      const res = await fetch(`${config.backendUrl}/incidents/${id}`, {
        method: 'DELETE',
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {})
        }
      });
      if (!res.ok) throw new Error('Failed to delete incident record');
      setRecords((prev) => prev.filter((r) => (r._id || r.id) !== id));
    } catch (err) {
      alert('Error deleting incident record');
    } finally {
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder={t('searchByDescriptionOrType')}
                  value={searchTerm}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('affectedAnimals')}</TableHead>
                ...
                {loading ? (
                  ...
                  filteredRecords.map((record) => (
                    <TableRow key={record._id || record.id}>
                      ...
                      <TableCell>{record.incidentDate ? record.incidentDate.slice(0, 10) : ""}</TableCell>
                      <TableCell>{record.reportedBy}</TableCell>
                      <TableCell className="max-w-xs truncate">{record.affectedAnimals}</TableCell>
                      <TableCell className="max-w-xs truncate">{record.incidentDescription}</TableCell>
                      <TableCell>
                        <Badge variant={getSeverityColor(record.severity) as any}>{record.severity}</Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant={getStatusColor(record.incidentStatus) as any}>{record.incidentStatus}</Badge>
                      </TableCell>
