...
import { Plus, Search, Download, Upload, Edit, Trash2, Eye, Calendar, Loader2 } from "lucide-react"
...
export default function VaccinationsPage() {
  ...
  const fetchRecords = async (page = 1, limit = itemsPerPage) => {
    ...
    try {
      ...
      const url = new URL(`${config.backendUrl}/vaccinations`)
      ...
      setRecords(Array.isArray(data.vaccinations) ? data.vaccinations : data.records || [])
      setTotalPages(data.totalPages || 1)
      setTotalRecords(data.total || (Array.isArray(data.vaccinations) ? data.vaccinations.length : 0))
      setCurrentPage(data.page || page)
    } catch (err) {
      setError('Error fetching vaccinations')
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredRecords = records.filter((record) => {
    ...
    const matchesType = filterType === "all" || record.vaccinationType === filterType
  ...
  const handleExportCSV = () => {
    ...
    a.href = url
    a.download = "vaccination-records.csv"
    a.click()
  }
  ...
  const handleDeleteRecord = async (id: string) => {
    ...
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
      const res = await fetch(`${config.backendUrl}/vaccinations/${id}`, {
        method: 'DELETE',
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {})
        }
      });
      if (!res.ok) throw new Error('Failed to delete vaccination record');
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          ...
          <Link href="/vaccinations/schedule">
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder={t('searchByTagOrVaccine')}
                  value={searchTerm}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t('vaccineName')}</TableHead>
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          <span className={isOverdue(record.nextDueDate) ? "text-red-600 font-medium" : ""}>
                            {record.nextDueDate ? record.nextDueDate.slice(0, 10) : ""}
                          </span>
